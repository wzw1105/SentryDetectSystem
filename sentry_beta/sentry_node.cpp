...
cv::Mat mapx_master, mapy_master, mapx_slave, mapy_slave;//用于initUndistortRectifyMap函数，计算无畸变和修正转换映射
...
void MastercompressedImageCallback(const sensor_msgs::CompressedImageConstPtr &msg)
{
    ...
    //remap：dst(x,y) = src(map1(x, y), map2(x, y));
...
int main(int argc, char** argv)
{
    ...
    //Define and get params 计算无畸变和修正转换关系 (1280, 1024)摄像头采集的图像尺寸
    ...
    //Create Engine
    trrt_detector infer_engine(model_file, onnx_file, 640);//640：input_size
    ...
    while(ros::ok())
    {
        ...
        //Infer and get the detection，得到yolo边界框、跟踪参数
        ...
        if(v_yolo_flag)
        {
            ...
            {
                cv::flip(master_warp_frame, yolo_master_out, 0);//0 沿X轴翻转
                cv::flip(slave_warp_frame, yolo_slave_out, 1);  //1 沿Y轴翻转
                //绘制目标检测的边界框、类别、机器人旋转角度、置信度
                for(int i = 0;i<master_yolo_boxes.size();i++)
                {   
                    cv::Scalar color = colors[master_yolo_boxes[i].cate/2];
                    cv::rectangle(yolo_master_out, cv::Point(master_yolo_boxes[i].x1, 480-master_yolo_boxes[i].y1), cv::Point(master_yolo_boxes[i].x2, 480-master_yolo_boxes[i].y2), cv::Scalar(0,255,0), 2);
